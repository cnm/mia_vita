...
/* Function Headers*/
void release_mem(unsigned int mem_addr, unsigned int memory_size);
...
int request_mem(unsigned int mem_addr, unsigned int size);
...
void request_memory_regions(void){
    ...
    new_addr = request_mem(GPIOA_EN_ADDRESS, MEMORY_SIZE);
    i = *(unsigned int *)(new_addr);
