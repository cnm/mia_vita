...
/*static unsigned long base = 0x3bc;*/
 ...
 */
int short_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
  printk(KERN_INFO "Did a read!!!!\n");
                        ...
                        size_t count, loff_t *f_pos)
{
  ...
  printk(KERN_INFO "Short Base: %lu\n", short_base);
  printk(KERN_INFO "MINOR: %d\n", minor);
  printk(KERN_INFO "PORT: %lu\n", port);
                      ...
                      loff_t *f_pos)
{
  ...
  printk(KERN_INFO "##############I'm writing in i write \n");
...
irqreturn_t short_interrupt(int irq, void *dev_id)
{
  ...
  printk(KERN_EMERG "short: no irq reported by probe\n");
  do_gettimeofday(&tv);
...
{
  printk(KERN_INFO "Short wq interrupt \n");
...
{
  printk(KERN_INFO "Short do work\n");
  short_do_tasklet(0);
}
...
int short_init(void)
{
  ...
  printk("'mknod /dev/short0 c %d 0'.\n", result);
  if (result < 0) {
      printk(KERN_INFO "short: can't get major number\n");
      printk(KERN_INFO "probably have to do mobprobe -r parport\n");
  ...
  if (short_irq >= 0) {
      ...
      if (result) {
          ...
          printk(KERN_INFO "Enabled paralel port 2\n");
